...
class Router:
    ...
    def converge_tables(self, router_in):
        ...
        '''for neighbor in self.cost_D:
            interface = self.cost_D[neighbor].keys()
                ...
                if(not self.cost_D[entry] == i and not str(entry)[:1] == 'H'):
                    if (self.rt_tbl_D[dest][entry] < lowest):
                        ...
                        for face in self.cost_D[entry]:
                            interface = int(face)
            print("Taking path %d from %s to %s" % (interface, self.name, self.cost_D[dest][interface]))
            ...
            if(not (self.name == 'RA' and j == 0) and not (self.name == 'RB' and j == 1)):
                for destination in self.rt_tbl_D:
                    for router in self.rt_tbl_D[destination]:
                        ...
                        p = NetworkPacket(0, 'control', str(destination)+self.name+str(self.rt_tbl_D[destination][router]))
                        try:
